...
dataset = load_dataset('json', data_files={'train': os.path.join(root, "train.json"), 'dev': os.path.join(root, "dev.json"), 'test': os.path.join(root, "test.json")}, field='data', cache_dir='Cache')
print(dataset)
...
class HierBertForTextClassfication(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None):
        hidden = self.dropout(self.hier_encoder(input_ids=input_ids, attention_mask=attention_mask)[1])
...
def compute_metrics(p, threshold=0.5):
    metrics = {}
    preds = (p.predictions > threshold).astype(np.float)
    refs = p.label_ids
