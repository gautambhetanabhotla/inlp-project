...
class BM25(object):
    ...
    def fit(self, X):
        ...
        # Pre-compute and cache corpus matrix once — avoids re-transforming all
        # candidates on every single query call (was the main bottleneck).
        self._X_raw  = super(TfidfVectorizer, self.vectorizer).transform(X)
        self._len_X  = self._X_raw.sum(1).A1
        self._X_csc  = self._X_raw.tocsc()   # CSC format needed for column slicing
        self.avdl    = self._X_raw.sum(1).mean()
        # Doc-length normalisation term is constant across queries
        self._dl_norm = self.k1 * (1 - self.b + self.b * self._len_X / self.avdl)

    def transform(self, q, X=None):
        """ Calculate BM25 between query q and cached corpus (X ignored, kept for API compat). """
        k1 = self.k1
        q, = super(TfidfVectorizer, self.vectorizer).transform([q])
        assert sparse.isspmatrix_csr(q)

        # Slice only the columns relevant to this query
        X_csc = self._X_csc[:, q.indices]
...
# Vectorized fill — avoids slow row-by-row iloc (was ~10s/row on large corpus)
score_rows = []
for query in sim_df[column_name].values:
    score_rows.append([bm_25_results_dict[query][int(c)] for c in column_candidates])
